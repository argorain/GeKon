    ...
    std::vector <candidate_t> c_convex(candidate_t X, candidate_t Y) {
        ...
        for (int j = 0; j < 2; ++j) {
            ...
            for (int k = 0; k < rows * cols; ++k) {
                int r = k / cols;
                int c = k % cols;
                kids[j].at<ker_num_t>(r, c) =
                        rand_vec[k] * X.at<ker_num_t>(r, c) + (1 - rand_vec[k]) * Y.at<ker_num_t>(r, c);
            }
        }
    ...
    void m_dynamic(candidate_t &X, unsigned int t, unsigned int T) {
        ...
        for (int r = 0; r < rows; ++r) {
            for (int c = 0; c < cols; ++c) {
                if (random(0, 1) < m_swap_prob) {
                    X.at<ker_num_t>(r, c) =
                            X.at<ker_num_t>(r, c) * (1 - pow(random(0, 1), pow(float(1) - float(t) / T, m_dynamic_B)));
                }
            }
        }
    }
